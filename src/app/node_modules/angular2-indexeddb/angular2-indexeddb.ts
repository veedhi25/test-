...
export class AngularIndexedDB {
    ...
    validateStoreName(storeName: string):boolean{
        let self = this;
        return self.dbWrapper.validateStoreName(storeName)
    }

    getMaxDate(storeName: string, indexName: string, cursorCallback: (evt: any) => void) {
        let self = this;
        let promise = new Promise<any>((resolve, reject) => {
            self.dbWrapper.validateBeforeTransaction(storeName, reject);
            let transaction = self.dbWrapper.createTransaction({
            ...
            request.onsuccess = (evt: Event) => {
                cursorCallback(evt.target.result.value);
                resolve();
            }
        });
        return promise;
