...
export class RFQListComponent extends AppComponentBase {
  settings = {
    mode: 'external',
    add: {
      addButtonContent: '',
  ...
  onAddClick() {
    try {
      this.router.navigate(["/pages/transaction/inventory/purchases/RFQ/add-RFQ-order", { mode: "add", returnUrl: this.router.url }]);
    } catch (ex) {
      console.log(ex);
      alert(ex);
    }
...
//   }
  ...
  onViewClick(event): void {
    try {
      this.router.navigate(["/pages/transaction/inventory/purchases/add-RFQ-order", { vchr: event.data.VCHRNO, mode: "view", returnUrl: this.router.url, div: event.data.DIVISION, phiscal: event.data.PhiscalID }]);
    } catch (ex) {
      console.log(ex);
      alert(ex);
    }
