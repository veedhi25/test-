...
export class BranchComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        try {
            ...
            if (!!this._activatedRoute.snapshot.params['name']) {
                let BID: string = "";
                BID = this._activatedRoute.snapshot.params['name'];

                this._branchservice.getBranch(BID)
                    .subscribe(data => {
                        if (data.status == 'ok') {
                            console.log(data);
                            this.branchObj=data.result;
    ...
    constructor(
        ...
        protected _branchservice: branchService, 
    ...
    onTypeChange(){
        this.showChanelOption = (this.form.get("type").value == 'retailer' || this.form.get("type").value == 'directparty') 
        if(!this.showChanelOption){
    ...
    onsubmit() {
        try {
            let saveModel = Object.assign(this.branchObj, {}, this.form.value)
            
            let sub = this._branchservice.saveBranch(this.mode, saveModel)
                .subscribe(data => {
                    if (data.status == 'ok') {
    ...
    typechange(value)
    {
       console.log("chefkc",this.branchObj,value); 
    }
}
