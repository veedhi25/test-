...
export class productHierarchyService {
   ...
   private get apiUrl():string{
     ...
     getProductHierarchy(bid:string) {
        let res={status:"error",result:""};
        let returnSubject:Subject<any>=new Subject();
        console.log("about to edit");
        let bodyData = {mode:'query',data:{BID:bid}};
        this.http.post(this.apiUrl +'/getProductHierarchy',bodyData,this.getRequestOption())
            .subscribe(response=>{
    ...
    public saveProductHeirarchy(mode: string, warehouse: any) {
        let res = { status: "error", result: "" }
        let returnSubject: Subject<any> = new Subject();
        let bodyData = { mode: mode, data: warehouse };
        console.log("company item"+bodyData);
        this.http.post(this.apiUrl + "/saveProductHierarchy", bodyData, this.getRequestOption())
            .subscribe(data => {
