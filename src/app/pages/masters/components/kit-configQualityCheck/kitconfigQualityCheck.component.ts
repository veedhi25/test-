...
@Component(
    {
        selector: 'kitconfigQualityCheck',
        templateUrl: './kitconfigQualityCheck.component.html',
        providers: [KitConfigQualityService, TransactionService],
        styleUrls: ["../../../modal-style.css"],
    }
)
export class KitConfigQualityComponent {
    mode: string = "add";
    router: Router;
    ...
    constructor(private masterService: MasterRepo,
        ...
        router: Router,
        private kitConfigService:KitConfigQualityService) {
    ...
    ngOnInit() {
        this.gridProductPopupSettings = Object.assign(new GenericPopUpSettings, {
            title: "ITEMS",
            apiEndpoints: `/getMenuitemWithStockPagedListQuality/${this.showStockedQuantityOnly}/${'all'}/${'NO'}/${this._trnMainService.userProfile.userWarehouse}`,
            defaultFilterIndex: 0,
            columns: [
                {
                    key: 'DESCA',
    ...
    onSaveClicked() {
        ...
        this.kitConfigService.saveKitQualityConfig(saveData).subscribe(res => {
            if (res.status == "ok") {
