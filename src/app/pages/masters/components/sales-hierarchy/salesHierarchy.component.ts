...
import { salesHierarchyService } from './salesHierarchy.service';
import { SalesHierarchy } from '../../../../common/interfaces/SalesHierarchy.interface';


@Component({
        selector: 'salesHierarchy',
        templateUrl: './salesHierarchy.component.html', 
        providers: [salesHierarchyService],
        styleUrls: ["../../../modal-style.css"],
 })  
export class SalesHierarchyComponent implements OnInit, OnDestroy {
    @ViewChild('childModal') childModal: ModalDirective;
    viewMode = false;
    DialogMessage: string = "Saving data please wait ..."
    mode: string = "add";
    modeTitle: string = '';
    salesHierarchyObj: SalesHierarchy = <SalesHierarchy>{};
        ...
        try {
           // this.getChanel();
            ...
            if (!!this._activatedRoute.snapshot.params['Id']) {
                ...
                this._salesHierarchyservice.getSalesHierarchy(BID)
                    .subscribe(data => {
                        if (data.status == 'ok') {
                            console.log(data);
                            let  salesHierarchyObj=data.result;

                            console.log("salesHierarchy"+salesHierarchyObj);
                            this.form.patchValue({
                                Id: salesHierarchyObj.Id,
                                Name: salesHierarchyObj.Name,
                                Address: salesHierarchyObj.Address,
                                Parent: salesHierarchyObj.Parent,
                            })
    ...
    constructor(
        ...
        protected _salesHierarchyservice: salesHierarchyService, 
    ...
    getParent(){
        this.masterService.getAllSalesHierarchyParent().subscribe(
            (res) => {
                this.chanels = res;
            }
        )
    ...
    onsubmit() {
        try {
            let saveModel = Object.assign(this.salesHierarchyObj, {}, this.form.value)
            
            let sub = this._salesHierarchyservice.saveSalesHierarchy(this.mode, saveModel)
                .subscribe(data => {
                    if (data.status == 'ok') {
