...
@Component({
    selector : 'result-balancesheet-dialog',
...
export class BalanceSheet{
    ...
    constructor(private masterService: MasterRepo,private authService:AuthService, public dialogref:MdDialogRef<BalanceSheet>, @Inject(MD_DIALOG_DATA) public data: DialogInfo){
         
     
        let userProfile = authService.getUserProfile();
        var startDate = userProfile.PhiscalYearInfo.BeginDate
        var endDate = userProfile.PhiscalYearInfo.EndDate

        console.log("fical year"+startDate+"end"+endDate);


        this.ReportParameters.DATE1 = new Date().toJSON().split('T')[0];
       
       // this.ReportParameters.DATE1 = startDate.toJSON().split('T')[0];
        this.changestartDate(this.ReportParameters.DATE1,'AD');

        
        this.ReportParameters.DATE2 = new Date().toJSON().split('T')[0];
       
       // this.ReportParameters.DATE2 = endDate.toJSON();    
        this.changeEndDate(this.ReportParameters.DATE2,'AD');

        this.ReportParameters.OPT_SHOWSUMMARY_REPORT = "0";
        this.ReportParameters.GroupLevel = "2";
        this.ReportParameters.OPT_ISVERTICAL_REPORT = "1";
    ...
    public DialogClosedResult(res) {
        ...
        this.ReportParameters.OSTOCK = "0";
        this.ReportParameters.CSTOCK = "0";

        this.reportdataEmit.emit({ status: res, data: {reportname:'Balance Sheet', reportparam:this.ReportParameters}});
