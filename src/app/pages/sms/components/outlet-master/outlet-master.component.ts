...
@Component({
  selector: "outlet-master",
  templateUrl: "./outlet-master.html",

})
export class OutletMasterComponent implements OnInit {
  ...
  onSaveClicked() {
    try {
      ...
      if ((this.outletname == "" || this.outletname == undefined)) {
      ...
      let sub = this.masterService.outletmaster(JSON.stringify(requestOject))
        .subscribe(data => {
          console.log(data, "RES");
          if (data.status == 'ok') {
            this.loadingService.hide();
            this.alertService.success("Outlet Saved Succesfully...");
            this.clearText();
          }
          else {
            this.loadingService.hide();
