...
@Component({
  selector: "ims-grid",
  templateUrl: "./ims-grid.component.html",
  styleUrls: ['./ims-grid.component.scss']
})
export class IMSGridComponent extends PagedListingComponentBase
  ...
  /** Input  */
  @Input() imsGridSettings: IMSGridSettings = undefined;
  ...
  getData() {
    ...
    let apiEndpoints = this.imsGridSettings.apiEndpoints; 
    let apiUrl = `${this.apiUrl}${apiEndpoints}?currentPage=${this.pageNumber
      }&maxResultCount=${this.pageSize}`;
    // console.log('apiurl', apiUrl);
    let additionalParamQuery = '';
    if (this.imsGridSettings.AdditionalParam && this.imsGridSettings.AdditionalParam.length) {
      this.imsGridSettings.AdditionalParam.forEach(x => {
  ...
  processData(response){
    ...
    this.totalItems = result ? result['totalCount'] : 0;
    this.itemList = result ? result['data'] : [];
  ...
  onValueChange(event, value, Field) {
    ...
    return this._http
      .get(requestEndPoint, this.getRequestOption())
      .subscribe(response => {
       this. processData(response);
      }, error => {
      });
  ...
  ngOnChanges(changes: any) {
    if (this.imsGridSettings != undefined) {
      // this.imsGridSettings = changes.imsGridSettings.currentValue;
      // console.log(this.imsGridSettings.actionKeys, "imsGridSettings");
      setTimeout(() => {
        this.itemList = [];
        this.isActive = true;
        this.selectedRowIndex = 0;
        this.pageSize = this.imsGridSettings.pageSize ? this.imsGridSettings.pageSize : this.pageSize;
        this.refreshPage();
        this.refresh();
      }, 10);
    }
  ...
  onActionClicked(actionString: string, index: number, actionData: ActionKeySettings) {
  ...
  outletconfigeditchild() {
    ...
    if (user.CompanyInfo.isHeadoffice == 1 || user.CompanyInfo.companycode == null) {
      this.imsGridSettings.actionKeys.map(a => {
        if (a.text == "Edit") {
          a.hidden = false
        }
      }
  ...
  ngOnInit() {
    this.outletconfigeditchild()
    this.outletconfigViewchild()
  }
}
