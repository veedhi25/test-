...
@Component(
    {
        selector: 'PBatchListOld',
        templateUrl: './PopBatchOld.html',
        styleUrls: ["../../../pages/Style.css", "../pStyle.css"],

    }
)
export class PopBatchOldComponent implements OnChanges {
    @Input() title: string;
    @Input() tabindex;
        ...
        if (changes && changes.BList) {
            ...
            this.BList.forEach(batch => {
                for (var attribute in batch.VARIANTDETAIL) {
                    if (this.headerKeys.indexOf(this._trnMainService.getVariantNameFromId(attribute)) == -1 && ['QTY', 'PRATE', 'MRP', 'SRATE'].indexOf(attribute) == -1) {
                        this.headerKeys.push(this._trnMainService.getVariantNameFromId(attribute))
                    }
                    if (this.keys.indexOf(attribute) == -1 && ['QTY', 'PRATE', 'MRP', 'SRATE'].indexOf(attribute) == -1) {
